...
calculate_DA <- function(x = sce, cluster_column = "primary", subject_id = "patient_id", condition = "condition", comparisons = NULL, method = "t.test", paired = FALSE){
  dtp <- colData(x)[c(cluster_column, condition, subject_id)] %>%
    table() %>% 
    as.data.frame() %>%
    dplyr::rename("counts" = Freq) %>%
...
plot_DS <- function(x = sce, assay = "exprs", markers = state_markers(x), cluster_column = "primary", subject_id = "patient_id", condition = "condition"){
...
calculate_DS <- function(x = sce, assay = "exprs", markers = state_markers(x), cluster_column = "primary", subject_id = "patient_id", condition = "condition"){
...
plot_density <- function(x = sce, dimred = "UMAP", color_by = "condition", xlim = NULL, ylim = NULL, bins = 10, colvec = NULL, contures = TRUE){
  
  df <- cbind(reducedDim(x, all_of(dimred)), colData(x)[all_of(color_by)]) %>% as.data.frame() %>% na.omit()
  
  if(is.null(xlim)){
    xlim <- c(min(df$UMAP1), max(df$UMAP1)) * 1.3
  }
  if(is.null(ylim)){
    ylim <- c(min(df$UMAP2), max(df$UMAP2)) * 1.3
  }
